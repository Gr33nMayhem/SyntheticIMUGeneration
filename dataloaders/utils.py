...
def components_selection_one_signal(t_signal, freq1, freq2, sampling_freq):
    ...
    freqs = np.array(
        sp.fftpack.fftfreq(t_signal_length, d=1 / float(sampling_freq)))  # frequency values between [-25hz:+25hz]
...
def PrepareWavelets(K, length=20, seed=1):
    ...
    for mw_temp in motherwavelets:
        if mw_temp.startswith('gaus') or mw_temp.startswith('mexh') or mw_temp.startswith('morl') or mw_temp.startswith(
                'cmor') or mw_temp.startswith('fbsp') or mw_temp.startswith('shan') or mw_temp.startswith('cgau'):
            pass
        else:
